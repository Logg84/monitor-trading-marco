...
PAGES = [
    ...
    ("pages/2b_Calendario.py", "Calendario",  "📅"),
    ("pages/3_Regime.py",      "Regime",      "🧭"),
    ("pages/4_COT.py",         "COT",         "🛢️"),
    ("pages/5_Settori.py",     "Settori",     "🏭"),
    ("pages/5_Simulazione.py", "Simulazione", "📈"),
]
...
def render_navbar(regime: str | None = None, title: str = "Terminale") -> None:
    ...
    cols = st.columns([1] * len(PAGES) + [0.45], gap="small")
    ...
    if cols[len(PAGES)].button(theme_label, use_container_width=True,
