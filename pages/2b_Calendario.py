...
if rows:
    # 'rows' è già nell'ordine di 'filtered' (già ordinato per data alla
    # riga 157): un secondo sort_values qui romperebbe la corrispondenza
    # indice-evento sulle date in pareggio, perché pandas non garantisce
    # la stabilità dell'ordinamento — e il click sulla riga usa proprio
    # filtered[rows_sel[0]] per recuperare il dettaglio.
    df_view = pd.DataFrame(rows).reset_index(drop=True)
