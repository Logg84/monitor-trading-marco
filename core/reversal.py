...
- analyze_ticker: pacchetto completo per un ticker (incl. Wyckoff + chiavi settore/sotto-settore, che sono SOLO informative: le regole di ingresso e di uscita non le leggono)
- auto_populate: promuove 🟡/🟢 dello screening in watchlist 🤖 (write-through GitHub)
- prune_watchlist: uscite automatiche (🤖 e 👤) con persistenza 5 chiusure (write-through) e MESSAGGIO CON CAUSA REALE (quale ramo della regola è scattato).
CIRCUITO DI PROTEZIONE: il pruning non pubblica mai una watchlist che resterebbe vuota (svuotamenti totali sono quasi sempre bug, non mercato).

Regole uscita (punti como da reversal_state, 🟡 a ≥2):
...
def analyze_ticker(ticker: str) -> dict | None:
    ...
    # Risolto: zones[0]["center"] invece di zones["center"] per evitare TypeError su lista
    poc_price = zones[0]["center"] if zones else None
    wyk = wyckoff_analysis(df, poc_price=poc_price)
    # Chiave di settore: informativa (etichette, note, raggruppamenti). NON entra in reversal_state né in _check_exit_conditions.
    try:
...
def _check_exit_conditions(df, zones, anchors, D, E, origin, min_lvl, points_threshold=2, dd_threshold=-20):
    """Verifica condizioni di uscita sulle ultime 5 chiusure. Versione ottimizzata: calcola RSI/ATR/medie una sola volta. Ritorna (bad, motivo): motivo descrive la CAUSA REALE dell'uscita (quale ramo della regola è scattato, con i valori odierni)."""
    close = df["Close"]
    n = len(close)
    if n < 30:
        return False, ""
    ...
    for i in idxs:
        ...
        cross = any(bool(above[-j]) and not bool(above[-j - 1]) for j in range(1, min(5, i) + 1)) if i >= 1 else False
        ...
        if origin == "auto":
            ...
            if not (pts < points_threshold and min_lvl is not None and price < min_lvl):
                return False, ""
    if origin == "manual":
        return True, (f"sotto soglia candidato ({pts_last}/6 < {points_threshold}) e chiusura {price_last:.2f} sotto il livello minimo inserito ({min_lvl:.2f}) per 5 chiusure consecutive")
    if dd_days == 5:
        return True, (f"tornato sopra il ritracciamento minimo: drawdown {dd_last:+.1f}% (> {dd_threshold}%) per 5 chiusure consecutive")
    if pts_days == 5:
        return True, (f"sotto soglia candidato: {pts_last}/6 punti (< {points_threshold}) per 5 chiusure consecutive")
    return True, (f"causa mista su 5 chiusure: {dd_days + both_days}× tornato sopra il ritracciamento minimo, {pts_days + both_days}× sotto soglia candidato (DD oggi {dd_last:+.1f}%, punti oggi {pts_last}/6)")

def prune_watchlist(analyses: dict | None = None) -> list[tuple[str, str]]:
    """Rimuove entry che soddisfano le condizioni di uscita. Ritorna [(ticker, motivo_reale)]. Se analyses è fornito (dict ticker→risultato analyze_ticker), li riusa invece di ricalcolare ogni entry. REGOLA: le entry con target_date non vengono mai rimosse."""
    entries = load_watchlist()
    removed = []
    for e in entries:
        ...
        D = a["rev"]["flags"]["D"]
        E = a["rev"]["flags"]["E"]
        bad, motivo = _check_exit_conditions(a["df"], a["zones"], a["anchors"], D, E, e["origin"], min_lvl)
        if bad:
            try:
    ...
    if removed:
        ...
        if remaining > 0:
            ...
            st.warning("Pruning avrebbe svuotato completamente la watchlist: pubblicazione su GitHub bloccata (circuito di protezione). Verifica core/watchlist_io.py e le regole di uscita.")
    return removed
