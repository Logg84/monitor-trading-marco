...
def wyckoff_analysis(df: pd.DataFrame, poc_price: float | None = None) -> dict:
    ...
    # ── CONFIDENCE ─────────────────────────────────────────
    confidence = "alta"
    # Completezza dati sugli ultimi 200 giorni (non i primi 200: con
    # df più corto di 200 barre "range(1,200)" sforava l'indice e
    # comunque guardava l'inizio della serie invece della coda recente).
    recent_close = close.tail(200)
    missing_days = int(recent_close.notna().sum())
