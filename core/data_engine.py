...
sanitizzazione ticker indici (regex + mappa anti doppio-suffisso), CONTESTO DI
SETTORE (core/sectors.py: etichetta, stato 0-100, breadth EW−CW, Priorità —
puramente informativo: non entra in reversal_state né nelle regole di uscita).
...
from core.sectors import (priorita, sector_cell, sector_label, sector_of,
                          sector_rows, sub_of, sub_rows, vento)
...
@st.cache_data(ttl=3600, show_spinner=False)
def download_ohlcv_chunked(tickers: tuple, period: str = "2y",
...
def screening(tickers: list[str], log=None, progress_cb=None) -> tuple[pd.DataFrame, dict]:
    ...
    # Contesto di settore: UNA batch di download per tutti gli ETF dei registri
    # (cache 1h). Se non è disponibile — runner senza rete e senza cache del
    # repo — le colonne restano "—": mai inventare un valore.
    try:
    ...
    _log(f"Sector rotation: {len(sec_rows)} settori GICS + {len(sub_rows_map)} "
         f"sotto-settori ({sec_src}).")
    rows = []
    for i, t in enumerate(tickers):
        ...
        try:
            ...
            if len(c) < 80:
                continue
            sub = pd.DataFrame({"Open": o, "High": h, "Low": l,
                               "Close": c, "Volume": v})
            ...
            # Contesto di settore: UN SOLO blocco difensivo. Se la lettura di
            # settore fallisce il titolo NON viene scartato — l'
            # `except Exception: continue` in fondo al ciclo taglierebbe righe in
            # silenzio. Qui si resta su "—"/n/d = "dati non disponibili", mai
            # "settore debole".
            ...
            try:
                ...
                if sec_key:
                    ...
                    # ⚠️ = segnale attivo su settore in calo: il segnale vale,
                    # l'ingresso è contro-trend di settore (nota, non filtro).
                    if rev["kind"] and sec_vento == "contro":
                        sec_label = f"⚠️ {sec_label}"
                    if _srow:
                        sec_etf = " · ".join(x for x in (_srow.get("cw"),
                                                         _srow.get("ew")) if x)
            ...
            rows.append({
                ...
                # contesto di settore (informativo): settore GICS + stato 0-100,
                # Δ EW−CW, vento, Priorità (Bottom + bonus settore ±10) e il
                # sotto-settore S&P Select Industry dove esiste
                "Settore": sec_label,
    ...
    # Copertura della classificazione di settore: quanti titoli classificati
    # ( Yahoo non sempre ha settore/industria per le mid cap europee: la
    # copertura va dichiarata, non implicita).
...
def build_universe() -> list[str]:
    """Universo per l'analisi libera: costituenti degli indici + i benchmark di
    settore: un ETF è un titolo a tutti gli effetti e le sue zone VWAP restano
    valide: così 'è in forza il settore o il singolo titolo?' si può leggere
    sullo stesso strumento che genera la lettura di settore."""
