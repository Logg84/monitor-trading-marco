...
def add_entry(ticker: str, origin: str = "manual",
              poc: float | None = None, notes: str = "",
              target_date: str | None = None,
              sector: str | None = None, sub: str | None = None) -> list[dict]:
    """Crea una entry. `sector` è una CAMPO INFORMATIVO (chiave di core.sectors):
    serve solo a raggruppare/leggere la watchlist, non partecipa ad alcuna regola
    di ingresso né di uscita (le uscite restano solo in prune_watchlist).
    Se non passato, prova a dedurlo (info Yahoo, cache 24h); se il dato non è
    disponibile resta None → la UI mostra "settore n/d", mai un valore inventato."""
    entries = load_watchlist()
    ...
    if sector is None or sub is None:
        try:
            ...
            sector = sector or sector_of(ticker)
            sub = sub or sub_of(ticker)
        except Exception:
            pass
    poc_origin = None
    if poc is not None:
...
def update_target_date(ticker: str, target_date: str | None) -> list[dict]:
    entries = load_watchlist()
    for e in entries:
        if e["ticker"] == ticker:
...
def reconcile(entries: list[dict],
    ...
    """
    Aggiorna i campi derivati (vwap, poc auto, settore/sotto-settore se
    mancanti) e basta.
    ...
    for e in entries:
        ...
        if e["origin"] == "auto":
            ...
            e["poc_origin"] = "auto"
        # campi informativi di settore: si riempiono una volta sola, non si
        # cancellano (e restano fuori da ogni regola di uscita)
        for kk in ("sector", "sub"):
            if not e.get(kk) and m.get(kk):
                e[kk] = m[kk]
        out.append(e)
