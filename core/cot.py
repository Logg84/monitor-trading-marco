...
- Managed Money: moltiplicatore ×1.5 (era ×2) per segnali meno aggressivi
- Soglie divergenze: specifiche per asset class (metals 4-5%, energy 7-8%)
- FX thresholds: 85/15 (era 80/20) per solo estremi veri
"""
...
# CORREZIONE (Settembre 2026): Soglie dinamiche per asset class
# Ogni mercato ha volatilità diversa, una soglia fissa non ha senso
TH_PX_DEFAULT = 0.025  # default 2.5%

# Mappa asset class → soglia specifica
TH_PX_BY_CLASS = {
    # Metals (oro, argento, rame, platino)
    "GOLD": 0.04, "SILVER": 0.05, "COPPER": 0.05, "PLATINUM": 0.05,
    # Energy (petrolio, gas)
    "WTI": 0.07, "BRENT": 0.07, "NG": 0.08,
    # Agriculture (grano, mais, soia, ecc.)
    "CORN": 0.05, "WHEAT": 0.05, "SOYBEANS": 0.05,
    "SOYBEAN_OIL": 0.04, "SOYBEAN_MEAL": 0.04,
    "COTTON": 0.05, "OJ": 0.06, "ROUGH_RICE": 0.04,
    # Livestock
    "LIVE_CATTLE": 0.04, "LEAN_HOGS": 0.05,
    # Other
    "LUMBER": 0.06,
}

def _get_threshold(symbol: str) -> float:
    """Restituisce la soglia di variazione prezzo specifica per l'asset."""
    return TH_PX_BY_CLASS.get(symbol, TH_PX_DEFAULT)
...
def divergenze(arr: list, px: list, sym: str = "") -> list[dict]:
    ...
    for k, campo in (("prod", "prod"), ("mm", "mm"), ("swap", "swap")):
        ...
        per_i = []
        # CORREZIONE (Settembre 2026): usa soglia specifica per l'asset
        th_px = _get_threshold(sym) if sym else TH_PX_DEFAULT
...
def fx_pair_state(pair: str, fx: dict) -> dict | None:
    ...
    # CORREZIONE (Settembre 2026): Soglie più strette per evitare falsi segnali
    # 85/15 invece di 80/20: solo estremi veri generano segnali
    # Questo riduce i segnali "watch" e "crowded" non informativi
    base_estrema_long = pB > 85
    base_estrema_short = pB < 15
    quote_estrema_long = pQ > 85
    quote_estrema_short = pQ < 15
...
def regime_scores(payload: dict | None) -> dict | None:
    ...
    return {
        # CORREZIONE (Settembre 2026): Moltiplicatore ridotto da ×2 a ×1.5
        # Managed money è trend-following, non mean-reverting puro
        # Un moltiplicatore troppo alto genera segnali troppo aggressivi
        ...
        "managed_money": float(np.clip((50 - pM_avg) * 1.5, -100, 100)),
        "producers": float(np.clip(_SEGNO_PROD * (pP_avg - 50) * 1.5, -100, 100)),
