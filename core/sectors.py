...
e Priorità (Bottom + bonus settore ±15), che serve a ORDINARE, non a decidere.

Punteggio di settore (0-100, pesi fissi e dichiarati):
  30% trend (sopra SMA50 / SMA200 della gamba cw) · 20% momentum 3m ·
...
- Soglie stato: FORTE ≥70, IN MIGLIORAMENTO ≥55, NEUTRO ≥40, DEBOLE ≥25, IN CALO <25
- Pesi score: trend 30% (era 25%), momentum 3m 20% (era 25%)
- Bonus settore: ±15 (era ±10) per dare più peso al contesto
...
def stato_of(score: float | None, mom63: float | None,
    ...
    CORREZIONE (Settembre 2026): Soglie alzate per distinguere meglio gli stati.
    - FORTE ≥70 (era ≥65): solo settori veramente forti
    - IN MIGLIORAMENTO ≥55 (era ≥50): trend chiaro ma non estremo
    - NEUTRO ≥40 (era ≥35): zona centrale più ampia
    - DEBOLE ≥25 (era ≥20): settori chiaramente deboli
    - IN CALO <25 (era <20): solo settori molto deboli
    """
    if score is None or mom63 is None or above50 is None:
        return "n/d", "⚪"
    if score >= 70 and mom63 > 0:
        return "FORTE", "🚀"
    if score >= 55:
...
def _metrics_for(closes: pd.DataFrame, key: str, bench: pd.Series) -> dict:
    """Stato di un unita dei tre registri. Gamba di lettura (quella da cui
    ...
    if ok and a["mom63"] is not None and a["above50"] is not None:
        ...
        # CORREZIONE (Settembre 2026): Pesi score modificati
        # - Trend: 30% (era 25%) - il trend di lungo periodo è più predittivo
        # - Momentum 3m: 20% (era 25%) - meno peso al momentum a breve
        score = round(100 * (
            0.30 * (trend / 2.0)
            + 0.20 * _clip01(0.5 + (a["mom63"] or 0.0) / 30.0)
...
def bonus_sector(score: float | None) -> int:
    """
    CORREZIONE (Settembre 2026): ±15 invece di ±10 per dare più peso al contesto di settore.
    Un settore molto forte (score 80) dà +15 invece di +6.
    Un settore molto debole (score 20) dà -15 invece di -6.
    """
    if score is None:
        return 0
    return int(round(np.clip((score - 50.0) / 3.33, -15, 15)))
