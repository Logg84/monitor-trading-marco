...
def promuovi_auto_da_screener(
    ...
    è entro ±soglia da POC o VWAP 3M/1Y/4Y. Auto-pulizia dei titoli auto usciti
    dalla zona. Non tocca mai i manuali.
    Ritorna anche conteggi trasparenti: in_zona, saltati (manuali in zona),
    saltati_tickers (i loro nomi), per il pannello di feedback.
    """
    ...
    for _, row_s in df_screener.iterrows():
        ...
        if min_dist > soglia_trigger_pct:
            continue  # fuori zona: non promovibile

        in_zona += 1  # è in zona: lo conto comunque, per trasparenza
